    ...
    ...(preview ? [
      {
        ...
        click: () => {
          clipboardAccess.writeTextBoth(lastTranscript).catch((err) => log('error', `Copy last failed: ${err.message}`));
        },
      },
...
ipcMain.handle("copy-to-clipboard", async (event, text) => {
  await clipboardAccess.writeTextBoth(text);
  return true;
});

ipcMain.handle("output-text", (event, text, method) => textOutput.outputText(text, method));
